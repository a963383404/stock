        ...
        for s in a:
            ...
            while i < l - 3:
                if self.determineGold(item[i], item[i + 1], item[i + 2], item[i + 3]):
                    total = total + 1;
                    # 更新颜色
                    ShAllDetail.objects.filter(id=item[i]['id']).update(color='#FFD700')
                else:
                    if item[i]['close'] > item[i]['open']:
                        # 更新颜色
                        ShAllDetail.objects.filter(id=item[i]['id']).update(color='#FF0000')
                    else:
                        # 更新颜色
                        ShAllDetail.objects.filter(id=item[i]['id']).update(color='#00FF00')
                i = i + 1
